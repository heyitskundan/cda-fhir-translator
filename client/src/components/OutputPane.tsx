...
export function OutputPane({ result, error }: Props) {
  ...
  const isXml = result?.translated.trimStart().startsWith("<") ?? false;
  const highlighted = useMemo(() => {
    if (!result) return "";
    return isXml ? highlightCda(result.translated) : highlightJson(result.translated);
  }, [result, isXml]);
      ...
      >
        ...
        {!error && !result && (
          <div className="text-muted flex h-full items-center justify-center p-8 text-center text-sm">
            Paste C-CDA XML or a FHIR Bundle and translate to see the output here.
          </div>
        )}
