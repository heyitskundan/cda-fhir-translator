...
import { mapDischargeMedicationsSection } from "../../src/cda-to-fhir/sections/discharge-medications.js";
...
describe("mapDischargeMedicationsSection", () => {
  it("maps a substanceAdministration entry to a MedicationStatement tagged Discharge Medication", () => {
    ...
    const result = mapDischargeMedicationsSection(root, patientRef);

    expect(result.resources).toHaveLength(1);
    const med = result.resources[0] as { category?: { text?: string }[] };
    expect(med.category?.[0]?.text).toBe("Discharge Medication");
