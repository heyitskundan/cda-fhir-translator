...
/** Shared with medications-administered.ts, admission-medications.ts, and
 * discharge-medications.ts, whose entries share this same substanceAdministration
 * shape. `category` distinguishes them on the reverse direction. */
export function buildMedicationFromSubstanceAdministration(
  sa: CdaNode,
  id: string,
  patient: Reference,
      ...
      ? [
          compact({
            ...
            doseAndRate: doseValue
              ? [{ doseQuantity: compact({ value: Number(doseValue), unit: doseUnit }) }]
              : undefined,
          }),
        ]
      : undefined;
...
export function mapMedicationsSection(root: CdaNode, patientRef: Reference): SectionMapResult {
  return mapSection(root, patientRef, {
    ...
    build: (sa, id, patient) => buildMedicationFromSubstanceAdministration(sa, id, patient),
  });
}
